...
int minBuses(Graph g, int src, int dest){
	BFS bfs(&g);
	bfs.run(src);
	if(bfs.is_visited(dest))
		return bfs.min_dist(dest);
	return -1;
}
...
int main()
{
    Graph g(12,false);
    // cout<<g.n<<endl;
	// lets say that 1 - 20 nodes for bus 1
	// 21 - 40 nodes for bus 2
	// 41 - 60 nodes for bus 3
	// 61 - 80 nodes for bus 4
	// Bus 1
	g.add_edge(1,3,0);
	g.add_edge(3,7,0);
	// Bus 2
	...
	// connecting edges for bus change
	g.add_edge(7,27, 1);
	g.add_edge(1,41, 1);
	g.add_edge(30,50, 1);
	g.add_edge(50,70, 1);
