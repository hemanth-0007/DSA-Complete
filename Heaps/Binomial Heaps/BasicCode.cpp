...
if (_heap.size() <= 1)
	return _heap;
list<Node*> new_heap;
list<Node*>::iterator prev,cur,next;
prev = cur = next = _heap.begin();
...
while (prev != _heap.end())
{
	// if only one element remains to be processed
	if (cur == _heap.end())
	prev++;

	// If D(prev) < D(cur) i.e. merging of Binomial
	// Tree pointed by prev & cur is not possible
	// then move next in heap
	else if ((*prev)->degree < (*cur)->degree)
	{
	prev++;
	cur++;
	if(next!=_heap.end())
	...
	// if D(prev),D(cur) & D(next) are same i.e.
	// degree of three consecutive Binomial Tree are same
	// in heap
	else if (next!=_heap.end() &&
		(*prev)->degree == (*cur)->degree &&
	...
	{
	Node *temp;
	*prev = mergeBinomialTrees(*prev,*cur);
	cur = _heap.erase(cur);
	if(next != _heap.end())
		next++;
	}
